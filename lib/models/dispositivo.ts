...
export type AlertaDispositivo = {
  timestamp: Date;
  tipoAlerta: 'consumoExcesivo' | 'bajaTension' | 'sobrecargaCircuito' | 'fallaConexion' | 'otro';
  mensaje: string;
  esResuelta: boolean;
...
export type ComandosDispositivo = {
  timestamp: Date;
  tipoComando: 'reinicio' | 'calibracion' | 'actualizacion' | 'cambioModo' | 'otro';
  comando: string;
...
export interface IDispositivo extends Document {
  ...
  modelo: string;
  fabricante: string;
  tipoDispositivo: 'medidorInteligente' | 'reguladorVoltaje' | 'monitorizador' | 'otro';
  numeroSerie: string;
  version: {
    hardware: string;
    firmware: string;
  ...
  configuracion: {
    modoOperacion: 'normal' | 'ahorro' | 'supervisionIntensiva' | 'balanceCarga';
...
const DispositivoSchema = new Schema<IDispositivo>({
  ...
  alertas: [{
    ...
    tipoAlerta: {
      type: String,
      enum: ['consumoExcesivo', 'bajaTension', 'sobrecargaCircuito', 'fallaConexion', 'otro'],
      required: true
    },
    mensaje: {
      type: String,
  ...
  comandos: [{
    ...
    tipoComando: {
      type: String,
      enum: ['reinicio', 'calibracion', 'actualizacion', 'cambioModo', 'otro'],
      required: true
    },
    comando: {
      type: String,
...
DispositivoSchema.statics.obtenerDispositivosInactivos = async function(diasInactivo: number = 2) {
...
// Definir el modelo
export const Dispositivo = mongoose.models.Dispositivo || 
