...
export function Notificaciones({ reducida = false }: NotificacionesProps) {
  ...
  const { notifications: socketNotifications, markNotificationAsRead } =
    useSocket();
    ...
    const cargarNotificaciones = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const notificacionesFormateadas = data.notificaciones.map(
            (n: any) => ({
              id: n._id,
              tipo: n.tipo,
              titulo: n.titulo,
  ...
  const actualizarResumen = useCallback(() => {
    ...
    const alertasNoLeidas = notificaciones.filter(
      (n) => n.tipo === "alerta" && !n.leida
    ).length;

    const info = notificaciones.filter((n) => n.tipo === "info").length;
    const infoNoLeidas = notificaciones.filter(
      (n) => n.tipo === "info" && !n.leida
    ).length;

    const exito = notificaciones.filter((n) => n.tipo === "exito").length;
    const exitoNoLeidas = notificaciones.filter(
      (n) => n.tipo === "exito" && !n.leida
    ).length;
              ...
              .map((notif) => (
                <div
                  ...
                  className={`p-3 rounded-lg ${
                    !notif.leida ? "bg-gray-50 dark:bg-slate-900" : ""
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold">
                  {resumenNotificaciones.alertas.total}
                </h3>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold">
                  {resumenNotificaciones.info.total}
                </h3>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold">
                  {resumenNotificaciones.exito.total}
                </h3>
