...
export const FLASK_ENDPOINTS = {
  ...
  control: (action: string) => `/api/arduino/led/${action}`,
  stats: "/api/arduino/stats", // futuro
  export: (format: string, days: number) =>
    `/api/arduino/export?format=${format}&days=${days}`,
};

// Configuración de auto-refresh por defecto
export const DEFAULT_AUTO_REFRESH: AutoRefreshConfig = {
  enabled: true,
  interval: 10000, // 10 segundos para reducir carga
  maxRetries: 3,
};
