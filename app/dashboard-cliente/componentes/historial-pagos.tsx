...
import { Download, CreditCard, Calendar, DollarSign, FileText, ArrowDownCircle, CheckCircle2, Clock } from 'lucide-react';
...
export function HistorialPagos({ reducida = false }: HistorialPagosProps) {
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 p-3 rounded-lg">
              <div className="flex justify-between items-start mb-1">
                <div>
                  ...
                  <div className="text-xl font-bold">${proximoPago.montoEstimado.toLocaleString('es-CL')}</div>
                </div>
              ...
              <Progress value={((30 - proximoPago.diasRestantes) / 30) * 100} className="h-1.5 mt-2" />
              ...
              {facturasEjemplo.slice(0, 3).map((factura, index) => (
                <div key={index} className="flex justify-between items-center p-2 border-b border-gray-100 dark:border-gray-800 last:border-0">
                  <div>
                    ...
                    <div className="text-xs text-gray-500">{factura.fechaPago}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-medium">${factura.monto.toLocaleString('es-CL')}</div>
                    <div className="text-xs text-gray-500">{factura.consumo} kWh</div>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              Factura de {proximoPago.periodo}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="text-right">
                ...
                <div className="text-lg font-bold">{proximoPago.fechaVencimiento}</div>
                <div className="text-sm text-gray-500">
            ...
            <Progress value={((30 - proximoPago.diasRestantes) / 30) * 100} className="h-2 mb-4" />
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-sm text-gray-500">Total anual {periodoSeleccionado}</div>
                  <div className="text-xl font-bold">
      ...
      <Tabs defaultValue="facturas" className="mb-6" onValueChange={setTabActiva}>
        <TabsList>
                ...
                <TableBody>
                  {facturasEjemplo.map((factura, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>${factura.monto.toLocaleString('es-CL')}</TableCell>
                      <TableCell>{renderizarEstadoFactura(factura.estado)}</TableCell>
          ...
          <Card>
            <CardContent className="pt-6">
              <Table>
                ...
                <TableBody>
                  {facturasEjemplo.filter(f => f.estado === 'pagada').map((factura, index) => (
                    <TableRow key={index}>
                      <TableCell className="font-medium">{factura.id}</TableCell>
                      <TableCell>{factura.periodo}</TableCell>
      ...
      <div className="bg-gray-50 dark:bg-slate-900 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
        <h3 className="text-lg font-semibold mb-3">Medios de pago disponibles</h3>
        <div className="grid md:grid-cols-3 gap-4">
          <div className="flex items-start gap-3">
            <CreditCard className="h-5 w-5 text-blue-600 mt-0.5" />
            <div>
              <h4 className="font-medium">Tarjeta de crédito/débito</h4>
              <p className="text-sm text-gray-500">Pague directamente con su tarjeta bancaria.</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Realice transferencias a nuestra cuenta corriente.</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Configure pagos automáticos mensuales.</p>
            </div>
          </div>
        </div>
      </div>
