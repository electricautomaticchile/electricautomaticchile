...
export default function FormularioContacto() {
  ...
  const subirArchivoS3 = async (file: File): Promise<string> => {
    try {
      ...
      formData.append('tipoDocumento', 'reporte_tecnico');
      formData.append('entidadRelacionada', 'cliente');
      formData.append('referenciaId', 'formulario-contacto');
      formData.append('entidadModelo', 'Cliente');
  ...
  const manejarEnvio = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      try {
        ...
        console.log('No se pudo resetear el formulario, pero los datos se enviaron correctamente');
      }
    } catch (error: any) {
      setEstado('error');
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className={`border-2 border-dashed rounded-md p-4 ${archivoSeleccionado ? 'border-orange-300 bg-orange-50' : 'border-gray-200'}`}>
                      <div className="flex flex-col items-center justify-center space-y-2">
                        <Upload className={`h-8 w-8 ${archivoSeleccionado ? 'text-orange-500' : 'text-gray-400'}`} />
                        <p className={`text-sm font-medium ${archivoSeleccionado ? 'text-orange-500' : 'text-gray-500'}`}>
                          {archivoSeleccionado ? 'Archivo listo para enviar' : 'Haz clic para seleccionar archivo'}
                        </p>
                        <p className="text-xs text-gray-500">Formato aceptado: PDF, DOC, DOCX, JPG, PNG (máx. 10MB)</p>
                        <input
                          ref={fileInputRef}
                          type="file"
                          id="file"
                    ...
                    {archivoNombre && (
                      <div className="flex items-center p-2 bg-orange-50 rounded border border-orange-100">
                        ...
                        <span className="text-sm text-gray-700 truncate flex-1">{archivoNombre}</span>
                        <Button
                          type="button"
                          variant="ghost"
                          size="sm"
                    ...
                    {subiendoArchivo && (
                      <div className="w-full mt-2">
                        ...
                        <p className="text-xs text-gray-500 mt-1 text-center">Subiendo archivo: {progresoSubida}%</p>
                      </div>
                    )}
                  </div>
                </div>
